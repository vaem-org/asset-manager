...
export default app => {
  const encoderIO = app.io.of('/encoder', null);
  const browserIO = app.io.of('/encoders', null);

  const router = new express.Router({});
  const json = express.json();

  app.use('/encoders', router);
  ...
  const sourceDone = async source => {
    ...
    try {
      ...
      if (app.config.slackHook) {
        request({
          url: app.config.slackHook,
          method: 'POST',
          json: {
  ...
  router.post('/start-job', json, api(async req => {
    ...
    let videoParameters = file ? await getVideoParameters(source, _.get(file, 'audioStreams.length') ? file.audioStreams : null) : null;
    ...
    if (req.body.assetId) {
      ...
      asset = new Asset({
        ...
        source,
        audio,
        hls_enc_key: app.config.hlsEnc ? crypto.randomBytes(16).toString('hex') : null,
        hls_enc_iv: app.config.hlsEnc ? crypto.randomBytes(16).toString('hex') : null
      });
    }
    ...
    const rFrameRate = _.get(_.find(_.get(asset, 'videoParameters.ffprobe.streams'), {codec_type: 'video'}), 'r_frame_rate', '').split('/').map(i => parseInt(i));
    ...
    _.each(app.config.profiles, (profiles, width) => {
